...
class Solution():
    def topKFrequent(self, nums, k):
        ...
        # all from the index
        # Bucket sort!! it should be with the length of nums; two edge cases 1) Non repeated elements in this case 
        # the all the elements of the nums reside in the first inner loop. 2) all repeated in this case all there is just one 
        # element at the last inner list.
        bucket = [[] for _ in range(len(nums))]
        for key, value in dic_count_num.items():
            bucket[value - 1].append(key)
        
        result = []  
        # Traverse in reverse as we are looking for the last k more often!!
        for i in range(len(bucket) - 1, -1, -1):
            if len(bucket[i]) != 0:
                if len(result) < k:
                    # The difference between + and append is with append it will be
                    # [[2, -1]] but when it is added the elements will go inside the list
