...
class Solution(object):
    def myAtoi(self, str):
        ...
        import re
        str_temp = str.strip()
        # () for creating the group ^ means it starts with plus or minus sign or zero [] for
        # creating new class of chars * means contains zero or more.
        # the follwoing means a string that starts with signs or zero and it can be zero or more
        # and has one or more digits this is the group 1 followed by any chars that is not a digit (\D)
        str_reg = re.compile(r"(^[\+\-0]*\d+)\D*") 
        mo = str_reg.findall(str_temp)
        try:
            res = int(''.join(mo))
            if res > 2 ** 31 - 1 > 0:
                return 2 ** 31 - 1
