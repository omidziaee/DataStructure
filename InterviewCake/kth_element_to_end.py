...
def kth_to_last_node(k, head):
    if k == 0:
        raise IndexError("k can not be zero!")
    values = get_values_of_nodes(head)
    if len(values) < k:
        raise IndexError("the %i can not be greated than the len of liked list %i" %(k, len(values)))
            
    return values[-k]

# Return the kth to last node in the linked list
def get_values_of_nodes(head_of_list):
    values = []
    while head_of_list:
        values.append(head_of_list)
